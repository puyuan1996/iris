...
class Trainer:
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.ckpt_dir = Path(debug_dir + 'checkpoints')
        self.media_dir = Path(debug_dir + 'media')
        ...
        if not cfg.common.resume:
            config_dir = Path(debug_dir + '/config')
            config_path = config_dir / 'trainer.yaml'
            config_dir.mkdir(exist_ok=True, parents=False)
            shutil.copy(debug_dir_prefix + '/.hydra/config.yaml', config_path)
            wandb.save(str(config_path))
            shutil.copytree(src=(Path(debug_dir_prefix_2) / "src"), dst=debug_dir + "/src")
            shutil.copytree(src=(Path(debug_dir_prefix_2) / "scripts"), dst=debug_dir + "/scripts")
    ...
    def train_agent(self, epoch: int) -> None:
        ...
        if epoch > cfg_tokenizer.start_after_epochs:
            metrics_tokenizer = self.train_component(self.agent.tokenizer, self.optimizer_tokenizer, sequence_length=1,
                                                     sample_from_start=True, **cfg_tokenizer)
        ...
        if epoch > cfg_actor_critic.start_after_epochs:
            metrics_actor_critic = self.train_component(self.agent.actor_critic, self.optimizer_actor_critic,
                                                        ...
                                                        sample_from_start=False, tokenizer=self.agent.tokenizer,
